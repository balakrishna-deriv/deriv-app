(window["webpackJsonpsmartcharts"] = window["webpackJsonpsmartcharts"] || []).push([["flutter-chart-adapter"],{

/***/ "./chart_app/build/web/flutter.js":
/*!****************************************!*\
  !*** ./chart_app/build/web/flutter.js ***!
  \****************************************/
...
(function () {
  ...
  async function timeout(promise, duration, debugName) {
    ...
    const _clock = new Promise((_, reject) => {
      timeoutId = setTimeout(() => {
        reject(new Error("".concat(debugName, " took more than ").concat(duration, "ms to resolve. Moving on."), {
          cause: timeout
        }));
      }, duration);
    });
  ...
  class FlutterTrustedTypesPolicy {
     ...
     */
    constructor(validPatterns) {
      let policyName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "flutter-js";
      ...
      if (window.trustedTypes) {
        this.policy = trustedTypes.createPolicy(policyName, {
          createScriptURL(url) {
            ...
            console.error("URL rejected by TrustedTypes policy", policyName, ":", url, "(download prevented)");
          }
  ...
  class FlutterServiceWorkerLoader {
    ...
    loadServiceWorker(settings) {
      ...
      const {
        ...
        serviceWorkerUrl = "flutter_service_worker.js?v=".concat(serviceWorkerVersion),
        timeoutMillis = 4000
      } = settings; // Apply the TrustedTypes policy, if present.
      ...
      const serviceWorkerActivation = navigator.serviceWorker.register(url).then(this._getNewServiceWorker).then(this._waitForServiceWorkerActivation); // Timeout race promise
  ...
  class FlutterEntrypointLoader {
    ...
    didCreateEngineInitializer(engineInitializer) {
      if (typeof this._didCreateEngineInitializerResolve === "function") {
        this._didCreateEngineInitializerResolve(engineInitializer); // Remove the resolver after the first time, so Flutter Web can hot restart.


        this._didCreateEngineInitializerResolve = null; // Make the engine revert to "auto" initialization on hot restart.
    ...
    _createScriptTag(url) {
      ...
      scriptTag.type = "application/javascript"; // Apply TrustedTypes validation, if available.
  ...
  class FlutterLoader {
    ...
    async loadEntrypoint(options) {
      const {
        serviceWorker,
        ...entrypoint
      } = options || {}; // A Trusted Types policy that is going to be used by the loader.

      const flutterTT = new FlutterTrustedTypesPolicy(); // The FlutterServiceWorkerLoader instance could be injected as a dependency
      ...
      entrypointLoader.setTrustedTypesPolicy(flutterTT.policy); // Install the `didCreateEngineInitializer` listener where Flutter web expects it to be.
...
//# sourceMappingURL=flutter-chart-adapter-f60173.smartcharts.js.map